...
public class CatalogViewModelFilterTests
{
    ...
    {
        // "on" is a genuine case-insensitive substring of two titles
        // ("Computati**on**al", "Introducti**on**") but not "Linear Algebra".
        var result = CatalogViewModel.FilterEntries(SampleEntries(), "on").ToList();

        Assert.Equal(2, result.Count);
        Assert.Contains(result, e => e.Slug == "hst-508");
