...
public class CatalogViewModelLoadMoreTests
{
    ...
    public async Task LoadMoreAsync_Success_AppendsEntriesAndAdvancesOffset()
    {
        ...
        catalog.RequestedOffsets.Should().Equal(new[] { 0 }, "the first page is requested from offset 0");
        vm.DisplayedCourses.Should().HaveCount(2, "the loaded entries are appended to the list");
        ...
        vm.IsBusy.Should().BeFalse("the busy flag clears after loading");
    }
