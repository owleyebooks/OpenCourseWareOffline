...
- One reviewer also notes the copy hides that a bare course identifier works: placeholder could add "— or type the course's page name."
...
Manual Refresh button at the bottom (why refresh storage by hand?), no live progress, no per-item breakdown, Delete is per-course only — a watched lecture cannot be deleted without deleting the whole course.
