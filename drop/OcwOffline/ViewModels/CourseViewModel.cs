...
public partial class CourseViewModel : ObservableObject
{
    ...
    public CourseViewModel(IOcwScraperService scraper, ICourseDatabase db, IDownloadManager downloads)
    {
        _scraper = scraper;
        _db = db;
    ...
    private async Task FetchAsync()
    {
        ...
        try
        {
            ...
            // Course zip previously discarded here; now surfaced first
            // as a downloadable Artifact. See AUDIT_TRAIL v14.

            // Reconciles by SourceUrl/VideoUrl so re-Fetch updates existing
            // rows in place instead of duplicating. See AUDIT_TRAIL v4.
    ...
    {
        // See the matching guard in DeleteArtifactAsync — same
    ...
    // Wires the Pause button — backend (DownloadManager.Pause) and status
    // mapping were already correct, only the UI caller was missing.
    // No-ops outside InProgress. See AUDIT_TRAIL v13.
    [RelayCommand]
    ...
    private async Task DownloadArtifactAsync(Artifact artifact)
    {
        ...
        // Same pattern as FetchAsync's IsBusy guard. See AUDIT_TRAIL v6.
        if (artifact.DownloadStatus is DownloadStatus.InProgress or DownloadStatus.Extracting)
            return;
        ...
        try
        {
            ...
            if (artifact.FileType == ArtifactFileType.Zip)
            {
                ...
                try
                {
                    var zipFullPath = Path.Combine(AppPaths.Root, relativePath);
                ...
                {
                    // Download succeeded, only unpacking failed — stays
                    // Completed, not Failed, since re-downloading won't
                    // fix a bad archive or a full disk.
                    artifact.IsExtracted = false;
    ...
    private async Task DownloadLectureAsync(Lecture lecture)
    {
        ...
        // See the matching guard in DownloadArtifactAsync — same
    ...
    private void OnProgressChanged(DownloadProgress progress)
    {
        ...
        // across both platforms we hop back explicitly.
